var calculator=function () {
    ...
    var keys = [],//the array of keys inputed
        isAResult = false,//true if a certain input follows a calculation (=)
        resetDisplay = false, //true if screen just reset with AC
        display = [];//what is displayed in the lower area of the screen (demo2)
    ...
    function addKey(x) {
        ...
        if (keys.length === 0) {
            ...
            if (resetDisplay) {
                if ((toStr.length === 1) || ((toStr.length === 2) && ((prevStr === 'multiplied') || (prevStr === 'div')))) {
                    display = [];
                    resetDisplay = false;
                }
            }
            ...
            if (toStr.length > 1) {
                if ((toStr.length === 2) && ((prevStr === 'multiplied') || (prevStr === 'div'))) { //this allows for negative numbers after * or /
                    keys.push('-');
                    display.push('-');
                    nrDecimalPoints=0;
                } else {
